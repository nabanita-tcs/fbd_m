/*! 
 * jquery.a-tools-connect
 * 
 * Copyright (c) 2009 Andrey Kramarev(andrey.kramarev[at]ampparit.com), Ampparit Inc. (www.ampparit.com)
 * Licensed under the MIT license.
 * http://www.ampparit.fi/a-tools/license.txt
 *
 * Modified by: Nitesh @ Tricon Infotech
 * Modified from 1.4.1 version of a-tools. Modified for better handing of the caret position in IE browsers
 * Basic usage:
	...
	getSelection: function() {  // function for getting selection, and position of the selected text
		...
		if (document.selection) {
			...
			if (typeof(input.selectionStart) == "number") {
				...
				var totalNewLines = input.value.match(/\r/g) ? input.value.match(/\r/g).length : 0;
				re = input.createTextRange();
				rc = re.duplicate();
				rd = re.duplicate();
				re.moveToBookmark(s.getBookmark());
				rc.setEndPoint("EndToStart", re);
				rd.setEndPoint("StartToEnd", re);
				var newLinesBeforeBookMark = 0;
				var dummy = rc.text;
				for(lp=0; lp<dummy.length; lp++) {
					var foundAt = dummy.indexOf("\r");
					if(foundAt > -1) {
						newLinesBeforeBookMark++;
						dummy = dummy.slice(foundAt+1);
						
					}
				}
				var tmp = input.value.replace(rc.text,"").replace(rd.text, "").match(/\r/g);
				var newLinesAfterBookMark = 0;
				
				if(false && felag && felag.length) {
					start = input.value.length;
				} else {
					var newLinesAfterBookMarkMatch = rd.text.match(/\r/g);
					var charsAfterBookmark = rd.text ? rd.text.length : 0;
					var newLinesAfterBookMark = newLinesAfterBookMarkMatch ? newLinesAfterBookMarkMatch.length: 0;
					start = input.value.length - charsAfterBookmark;
					onLine = newLinesAfterBookMark;
			...
			return { start: start, end: start, text: s.text, length: 0, onLine: totalNewLines-onLine };
