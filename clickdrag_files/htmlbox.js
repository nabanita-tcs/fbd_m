...
$.fn.htmlbox=function(options){
	...
	// Default option
	var d={
	    toolbars:[["bold","italics","underlined"]],      // Buttons
	...
	/**
	 * update cursor pointer position on mousedown
	 */
	var clickUpdate = function(e){
		//console.log("@clickUpdate :: htmlbox");
		try{
			var textBoxContainer = $('#textToolContainer');
			...
			var iFrameId = '#'+selectedTextArea.attr('id')+'_html';
			if($(iFrameId).contents()[0].getSelection().anchorNode.length){
				textSelectionStart = $(iFrameId).contents()[0].getSelection().anchorOffset;
				textSelectionEnd = $(iFrameId).contents()[0].getSelection().anchorOffset;
				textNodeObj = $(iFrameId).contents()[0].getSelection().anchorNode;
			}else{
				var adjstmnt = 0;
				if($(iFrameId).contents()[0].body.innerHTML.indexOf('<br>') != -1){
					adjstmnt = 4;
				}
				var calc = $(iFrameId).contents()[0].body.innerHTML.length - adjstmnt;//4 for end br
				textSelectionStart = calc;
				textSelectionEnd = calc;
			}
		}catch(err){
			console.log("Error in clickUpdate :: htmlbox "+err.message);
		}		
	}
    //=====================================================================//
    //  METHOD: keyup                                                      //
	...
	this._init = function(is_init){
		...
		*/
		...
		try {
		   ...
		   $(iframe).css("width",parseInt(w)+15+'px').css("height",parseInt(h)+10+'px').attr("id",d.id+"_html").css("border","0");
		   $(iframe).css("border",'1px solid #AAAAAA');
		  // $(iframe).css("padding",'5px 5px 5px 10px');
		   ...
		   text = this.changeSubSuperscript(text);
		   ...
		   if(d.css.indexOf("background:")===-1){d.css+="body{background:white;}";}
		   if(d.css.indexOf("background-image:")===-1){
		       d.css=d.css+"body{'padding','5px 5px 5px 10px';background-position:top right;background-repeat:no-repeat;}";
		   }
		   ...
		   if(iframe.contentWindow.document.attachEvent){
		       ...
		       iframe.contentWindow.document.attachEvent("click",clickUpdate);
		       iframe.contentWindow.document.attachEvent("onkeyup",clickUpdate);
		   }else{
			   iframe.contentWindow.document.addEventListener("keyup",keyup,false);
			   iframe.contentWindow.document.addEventListener("click",clickUpdate,false);
			   iframe.contentWindow.document.addEventListener("keyup",clickUpdate,false);
			   iframe.contentWindow.document.addEventListener("blur",clickUpdate,false);
			   iframe.contentWindow.document.addEventListener("keydown",clickUpdate,false);
		   }
		   
		   var toolBarLen = d.toolbars[0].length;
		   var toolBarArray = d.toolbars[0];
		   for(var k = 0;k<toolBarLen;k++){
			   var toolBarId = '.'+toolBarArray[k]+'_text_tool';
			   var toolBarDiv = $(toolBarId);
			   toolBarDiv.off('click').on('click',function(){
				   global_hb[d.id].cmd(this.title,d); 
			   });
		   }
		   $(this).hide();
	    }catch(e){
	...
	//=====================================================================//
    //  METHOD: changeSubSuperscript                                                      //
    //========================== START OF METHOD ============================//
	
	this.changeSubSuperscript = function(targetString){
		console.log("@changeSubSuperscript :: htmlbox.full");
		try{
			targetString = targetString.replace(/<sub>/g, '&lt;sub&gt;');
			targetString = targetString.replace(/<\/sub>/g, '&lt;/sub&gt;');
			
			targetString = targetString.replace(/<sup>/g, '&lt;sup&gt;');
			targetString = targetString.replace(/<\/sup>/g, '&lt;/sup&gt;');
			
			targetString = targetString.replace(/<sp>/g, '&lt;sp&gt;');
			targetString = targetString.replace(/<\/sp>/g, '&lt;/sp&gt;');
			
			targetString = targetString.replace(/<sb>/g, '&lt;sb&gt;');
			targetString = targetString.replace(/<\/sb>/g, '&lt;/sb&gt;');
			
			return targetString;
		}
		catch(err){
			console.error("Error in changeSubSuperscript :: htmlbox.full "+err.message);
		}
	}
    ...
    //=====================================================================//
	    ...
	    * @return this
	    */
	this.cmd = function(cmd,x){
		d = x;//as the reference of d is getting lost while event bind
